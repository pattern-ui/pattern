...
export function useLocalStorage<T = string>({
      ...
      if (val instanceof Function) {
        setValue((current) => {
          ...
          window.dispatchEvent(
            new CustomEvent('pattern-local-storage', { detail: { key, value: val(current) } })
          );
          return result;
        });
      } else {
        ...
        window.dispatchEvent(
          new CustomEvent('pattern-local-storage', { detail: { key, value: val } })
        );
        setValue(val);
      }
    },
  ...
  useWindowEvent('pattern-local-storage', (event) => {
