...
describe('@pattern/core/Thumb', () => {
  ...
  it('shows label based on dragging and labelAlwaysOn props', () => {
    ...
    expect(labelAlwaysOn.querySelectorAll('.pattern-Slider-label')).toHaveLength(1);
    expect(dragging.querySelectorAll('.pattern-Slider-label')).toHaveLength(1);
    expect(labelOff.querySelectorAll('.pattern-Slider-label')).toHaveLength(0);
  });
