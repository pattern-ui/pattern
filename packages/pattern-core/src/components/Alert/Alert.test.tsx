...
describe('@pattern-ui/core/Alert', () => {
  ...
  it('renders close button based on withCloseButton prop', () => {
    ...
    expect(withCloseButton.querySelector('.pattern-Alert-closeButton')).toBeInTheDocument();
    expect(withoutCloseButton.querySelectorAll('.pattern-Alert-closeButton')).toHaveLength(0);
  });
  ...
  it('does not render title if title prop was not passed', () => {
    ...
    expect(container.querySelectorAll('.pattern-Alert-title')).toHaveLength(0);
  });
