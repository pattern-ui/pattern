...
installation: '@pattern/notifications'
license: MIT
---

import { NotificationDemos, NotificationsDemos } from '@pattern/demos';
...
```sh
npm install @pattern/notifications @pattern/core @pattern/hooks
```

Install with yarn:

```sh
yarn add @pattern/notifications @pattern/core @pattern/hooks
```
...
`@pattern/notifications` package exports the following functions:
...
All functions can be imported from `@pattern/notifications` package and can be used in any part of your application:
...
```tsx
import { showNotification, hideNotification } from '@pattern/notifications';
