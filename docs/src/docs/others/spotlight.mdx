...
import: "import { SpotlightProvider } from '@pattern-ui/spotlight';"
source: 'pattern-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@pattern-ui/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@pattern-ui/demos';
...
Note that if you are using [PatternProvider](/theming/pattern-provider/), `SpotlightProvider` must be used as its child:

```tsx
import { PatternProvider } from '@pattern-ui/core';
import { SpotlightProvider } from '@pattern-ui/spotlight';
...
You can setup multiple shortcuts, for example, Pattern documentation uses the following setup:
...
`@pattern-ui/spotlight` exports several functions which can be used to perform certain actions
...
You can import `SpotlightAction` type from `@pattern-ui/spotlight` package:
