...
import { SelectDemos, HighlightDemos, CardDemos, CalendarDemos, HooksDemos } from '@pattern-ui/demos';

## @pattern-ui/dates improvements
...
- All @pattern-ui/dates components that use Calendar no longer support `withSelect` and `yearsRange` props
...
```tsx
import { useInputState } from '@pattern-ui/hooks';
import { TextInput, NumberInput } from '@pattern-ui/core';
