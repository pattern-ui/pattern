...
- You can now change default locale that is used in all `@pattern-ui/dates` components with `dateLocale` on [PatternProvider](https://pattern-ui.design/theming/pattern-provider/)
