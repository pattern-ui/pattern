...
import { MediaQueryDemos, CreateStylesDemos } from '@pattern-ui/demos';

# Responsive styles

`@pattern-ui/core` package exports several components that can help you organize responsive styles without writing CSS.
...
`theme.breakpoints` are used in all responsive Pattern components. Breakpoints are set in px (rem, em and other values are not supported).
You can configure these values with [PatternProvider](/theming/pattern-provider/):
...
Most of Pattern components support `size` prop with `xs`, `sm`, `md`, `lg` and `xl` values.
...
All Pattern components support `sx` prop with which you can add styles (including responsive styles) to root component element:
...
If you want to add styles with `sx` prop to non-Pattern component, you can wrap it in [Box](/core/box/):
