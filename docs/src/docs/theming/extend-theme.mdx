...
```tsx
import { Button, Code, PatternProvider, Title } from '@pattern/core';
...
```tsx
import { PatternProvider, Box } from '@pattern/core';
...
```ts
import { Tuple } from '@pattern/core';

type CustomColors = 'primaryColorName' | 'secondaryColorName';

declare module '@pattern/core' {
...
import { Tuple, DefaultPatternColor } from '@pattern/core';
