...
By default, Pattern uses system fonts. You can change fonts and other text styles for headings, code and all other components:
...
Pattern supports 5 sizes for spacing, radius and shadows: xs, sm, md, lg, xl.
...
```tsx
import { Button, Code, PatternProvider, Title } from '@pattern-ui/core';
...
Pattern supports 5 sizes for breakpoints (`xs`, `sm`, `md`, `lg`, `xl`), which are used in [Grid](/core/grid/), [SimpleGrid](/core/simple-grid/) and other components.
...
Pattern uses Bootstrap's breakpoint values as defaults:
...
```tsx
import { PatternProvider, Box } from '@pattern-ui/core';
...
TypeScript will only autocomplete Pattern's default colors when accessing the theme. To add your custom colors to the PatternColor type, you can use TypeScript module declaration:

```ts
import { Tuple } from '@pattern-ui/core';

type CustomColors = 'primaryColorName' | 'secondaryColorName';

declare module '@pattern-ui/core' {
  export interface PatternThemeColorsOverride {
    colors: Record<CustomColors, Tuple<string, 10>>;
  }
}

// or if you want to "extend" standard colors
import { Tuple, DefaultPatternColor } from '@pattern-ui/core';

type ExtendedCustomColors = 'primaryColorName' | 'secondaryColorName' | DefaultPatternColor;
...
```ts
declare module '@pattern-ui/core' {
  export interface PatternThemeOther {
