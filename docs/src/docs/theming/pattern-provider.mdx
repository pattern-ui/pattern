...
interface PatternTheme {
  ...
  // Default dates formatting locale used in every @pattern/dates component
  datesLocale: string;
}
```
...
```tsx
import { PatternThemeOverride, PatternProvider } from '@pattern/core';
...
```tsx
import { PatternProvider, Text } from '@pattern/core';
...
```tsx
import { PatternProvider, Button, Badge } from '@pattern/core';
