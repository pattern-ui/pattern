...
import { TabsDemos, CreateStylesDemos } from '@pattern-ui/demos';

# createStyles

All Pattern components are built with [emotion](https://emotion.sh/) based css-in-js library.
We recommend using `createStyles` to create styles for the rest of your application
as it provides the most convenient way to utilize Pattern theme, but it is not required
...
defined via [PatternProvider](/theming/extend-theme/#breakpoints) or just static values:
...
Most of Pattern components support [Styles API](/theming/styles-api/).
You can override default component styles inline or using [PatternProvider](/theming/pattern-provider/)
the same way as with `createStyles`:

<Demo data={TabsDemos.stylesApi} />

## Using createStyles without @pattern-ui/core

You can use `createStyles` and `PatternProvider` without any other Pattern packages:
...
After installation you can import `createStyles` and other function from `@pattern-ui/styles` instead of `@pattern-ui/core` and use it separately:

```tsx
import { createStyles, PatternProvider, usePatternTheme } from '@pattern-ui/styles';
```

## Can I use other css-in-js libraries or SCSS with Pattern?

Yes, you can. But please note that Pattern already uses [emotion](https://emotion.sh/) and you may
