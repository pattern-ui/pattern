...
import { TabsDemos, CreateStylesDemos } from '@pattern/demos';
...
## Using createStyles without @pattern/core
...
After installation you can import `createStyles` and other function from `@pattern/styles` instead of `@pattern/core` and use it separately:

```tsx
import { createStyles, PatternProvider, usePatternTheme } from '@pattern/styles';
```
