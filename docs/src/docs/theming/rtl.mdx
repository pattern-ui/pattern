...
Pattern is built with [emotion](https://emotion.sh) which supports RTL languages with [stylis-plugin-rtl](https://github.com/styled-components/stylis-plugin-rtl).
...
Add `stylis-plugin-rtl` and `theme.dir` to your [PatternProvider](/theming/pattern-provider/):
...
function App() {
  return (
    <PatternProvider
      withGlobalStyles
      withNormalizeCSS
      emotionOptions={{ key: 'pattern', stylisPlugins: [rtlPlugin] }}
      theme={{ dir: 'rtl' }}
    >
      <YourApp />
    </PatternProvider>
  ...
  return (
    <PatternProvider
      ...
      emotionOptions={
        rtl
          ? // rtl cache
            { key: 'pattern-rtl', stylisPlugins: [rtlPlugin] }
          : // ltr cache
            { key: 'pattern' }
      }
    >
