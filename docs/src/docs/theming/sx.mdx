...
All Pattern components support `sx` prop. With `sx` you can add styles to component **root** element.
...
function. Styles added with `sx` will be parsed by [emotion](https://emotion.sh) and added to page head or specified target container in [PatternProvider](/theming/pattern-provider/).
...
Use a function instead of an object to subscribe to Pattern theme:
...
[Box](/core/box/) is an utility polymorphic component that can be used to style non Pattern components or elements with sx:
...
```tsx
import { Text, Box, CSSObject } from '@pattern-ui/core';

// styles object is compatible with every Pattern component
...
To store function styles in a variable you will need to use `PatternTheme` type in case you are using TypeScript:

```tsx
import { Text, Box, CSSObject, PatternTheme } from '@pattern-ui/core';

const styles = (theme: PatternTheme): CSSObject => ({
