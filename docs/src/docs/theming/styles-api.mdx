...
import { Slider as SliderStylesApi, Button as ButtonStylesApi } from '@pattern-ui/styles-api';

# Styles API

All Pattern components support styling of each component part with inline styles or by passing classes.
...
<Demo data={SliderDemos.control} configuratorProps={{ includeCode: false }} />
...
```tsx
import { Slider, createStyles } from '@pattern-ui/core';
...
## Styles API with PatternProvider

You can also use Styles API in PatternProvider with `styles` prop.
