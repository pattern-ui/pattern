...
props: ['Overlay']
import: "import { Overlay } from '@pattern-ui/core';"
source: 'pattern-core/src/components/Overlay/Overlay.tsx'
docs: 'core/Overlay.mdx'
styles: ['Overlay', 'single-node']
---

import { OverlayDemos, CardDemos } from '@pattern-ui/demos';
...
You can change overlay opacity (from 0 to 1), color (CSS color value, not connected to Pattern theme) and z-index (number).
...
```tsx
import { Overlay } from '@pattern-ui/core';
