...
import: "import { Accordion } from '@pattern-ui/core';"
source: 'pattern-core/src/components/Accordion/Accordion.tsx'
docs: 'core/Accordion.mdx'
styles: ['Accordion']
---

import { AccordionDemos } from '@pattern-ui/demos';
...
`AccordionProps` type exported from `@pattern-ui/core` is a generic, it accepts boolean type that
describes `multiple` state:

```tsx
import type { AccordionProps } from '@pattern-ui/core';
