...
import: "import { Accordion } from '@pattern/core';"
source: 'pattern-core/src/components/Accordion/Accordion.tsx'
docs: 'core/Accordion.mdx'
styles: ['Accordion']
---

import { AccordionDemos } from '@pattern/demos';
...
`AccordionProps` type exported from `@pattern/core` is a generic, it accepts boolean type that
describes `multiple` state:

```tsx
import type { AccordionProps } from '@pattern/core';
