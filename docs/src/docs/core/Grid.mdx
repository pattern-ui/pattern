...
props: ['Grid']
import: "import { Grid } from '@pattern-ui/core';"
source: 'pattern-core/src/components/Grid/Grid.tsx'
docs: 'core/Grid.mdx'
styles: ['Grid', 'single-node']
---

import { GridDemos } from '@pattern-ui/demos';
...
<ControlDemo data={GridDemos.growControl} />
...
<ControlDemo data={GridDemos.flexControl} />

## Responsive columns

Use breakpoint props (`xs`, `sm`, `md`, `lg`, `xl`) to make columns respond to viewport changes.
You can configure breakpoint values in [PatternProvider](/theming/extend-theme/#breakpoints).
