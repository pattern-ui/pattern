...
props: ['Input']
import: "import { Input } from '@pattern-ui/core';"
source: 'pattern-core/src/components/Input/Input.tsx'
docs: 'core/Input.mdx'
styles: ['Input']
---

import { InputDemos } from '@pattern-ui/demos';
...
and Input component as base for your own custom inputs with Pattern theme.

<ControlDemo data={InputDemos.control} />

## Variants

Input has 3 variants, all of which are available on all Pattern inputs.
...
If you want to add your own styles to input it's better to start from scratch rather than overriding Pattern styles,
use special `headless` variant which does not include any Pattern styles but still supports all other features: icon, right section, etc.:
