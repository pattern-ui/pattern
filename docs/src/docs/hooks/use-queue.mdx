...
import: "import { useQueue } from '@pattern-ui/hooks';"
docs: 'hooks/use-queue.mdx'
source: 'pattern-hooks/src/use-queue/use-queue.ts'
---

import { NotificationsDemos } from '@pattern-ui/demos';

## Usage

use-queue limits the amount of data in current state and place the rest of it in a queue.
For example, in [@pattern-ui/notifications](/others/notifications/) package amount of
...
```tsx
import { useQueue } from '@pattern-ui/hooks';
...
Example of use-queue hook usage in [Pattern notifications system](/others/notifications/). By default
