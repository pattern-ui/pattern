...
import: "import { useQueue } from '@pattern/hooks';"
docs: 'hooks/use-queue.mdx'
source: 'pattern-hooks/src/use-queue/use-queue.ts'
---

import { NotificationsDemos } from '@pattern/demos';

## Usage

use-queue limits the amount of data in current state and place the rest of it in a queue.
For example, in [@pattern/notifications](/others/notifications/) package amount of
...
```tsx
import { useQueue } from '@pattern/hooks';
