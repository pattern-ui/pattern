...
import: "import { useLocalStorage } from '@pattern-ui/hooks';"
docs: 'hooks/use-local-storage.mdx'
source: 'pattern-hooks/src/use-local-storage/use-local-storage.ts'
---
...
```tsx
import { useLocalStorage } from '@pattern-ui/hooks';
...
Pattern docs website uses this hook to store color scheme information:

```tsx
import { useLocalStorage } from '@pattern-ui/hooks';
import { ActionIcon, ColorScheme } from '@pattern-ui/core';
...
You can test this feature by opening 2 tabs with Pattern docs side by side and changing color scheme
