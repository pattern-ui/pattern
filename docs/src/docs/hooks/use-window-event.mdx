...
import: "import { useWindowEvent } from '@pattern-ui/hooks';"
docs: 'hooks/use-window-event.mdx'
source: 'pattern-hooks/src/use-window-event/use-window-event.ts'
---
...
import { useWindowEvent } from '@pattern-ui/hooks';
...
Search focus with `⌘ + K` on mac or `Ctrl + K` on windows and linux on Pattern docs website:
