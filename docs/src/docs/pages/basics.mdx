...
import { SliderDemos, CreateStylesDemos } from '@pattern/demos';
...
- `@pattern/hooks` – collection of 30+ react hooks for state and UI management.
- `@pattern/styles` – [emotion](https://emotion.sh/) based css-in-js library that is used in all Pattern components. Usually this package is installed automatically and exported from `@pattern/core` – there is no need to install it separately, [learn more about createStyles here](/theming/create-styles/).
- `@pattern/core` – core components library – 100+ components, exports everything from `@pattern/styles`.
- `@pattern/notifications` – a fully featured [notifications system](/others/notifications/).
- `@pattern/prism` – [code highlight](/others/prism/) built with [prism-react-renderer](https://github.com/FormidableLabs/prism-react-renderer).
- `@pattern/rte` – a Quill based [rich text editor](/others/rte/).
- `@pattern/dropzone` – manages [files drag 'n' drop](/others/dropzone/) of files to an area or the entire screen.
- `@pattern/ssr` – [server side rendering](/theming/ssr/) utilities.
- `@pattern/next` – components and ssr utils for [Next.js integration](/theming/next/).
- `gatsby-plugin-pattern` – [Gatsby plugin](/theming/gatsby/) to setup ssr.
- `@pattern/eslint-config` – ESLint and Prettier configuration that the Pattern project uses.
...
import { Button, Paper, TextInput } from '@pattern/core';
...
@pattern/core package export types to help you build components and styles with TypeScript:
...
```tsx
import type { ButtonProps } from '@pattern/core';
```

### Polymorphic components types

@pattern/core package exports `PolymorphicRef` and `PolymorphicComponentProps` types to help you build
custom components with ref forwarding:

```tsx
import { forwardRef } from 'react';
import { PolymorphicRef, PolymorphicComponentProps } from '@pattern/core';
import { Link } from 'react-router-dom';
