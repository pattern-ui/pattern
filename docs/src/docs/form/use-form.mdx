...
import: "import { useForm } from '@pattern-ui/form';"
...
`@pattern-ui/form` package does not depend on any other libraries, you can use it with or without `@pattern-ui/core` inputs:
...
`setFieldValue` can be used with all `@pattern-ui/core` inputs, example with [TextInput](/core/text-input/) component:
...
// value – input value
// onChange – input onChange function, works with all Pattern and regular inputs
...
// onChange – input onChange event, works with Pattern Checkbox, Switch and regular input[type="checkbox"]
// error – error message if input is invalid
```
