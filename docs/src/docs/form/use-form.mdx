...
import: "import { useForm } from '@pattern/form';"
...
`@pattern/form` package does not depend on any other libraries, you can use it with or without `@pattern/core` inputs:
...
`setFieldValue` can be used with all `@pattern/core` inputs, example with [TextInput](/core/text-input/) component:
