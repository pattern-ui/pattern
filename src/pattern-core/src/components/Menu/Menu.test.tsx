...
describe('@pattern/core/Menu', () => {
  ...
  itSupportsFocusEvents(Menu, defaultProps, '.pattern-ActionIcon-root');
  itSupportsSystemProps({
    component: Menu,
    props: defaultProps,
  ...
  it('opens menu when controlled is clicked and trigger prop is "click"', async () => {
    ...
    expect(container.querySelector('.pattern-Menu-body')).toBe(null);
    await actAsync(() => userEvent.click(screen.getByLabelText('test-menu')));
    expect(container.querySelector('.pattern-Menu-body')).toBeInTheDocument();
  });
